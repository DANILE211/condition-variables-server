 ...
 *                              get_mucod_id                                 *
 *===========================================================================*/
endpoint_t get_mucod_id()
{
/* get MUCOD's endpoint,
 * the reason that we directly get the endpoint
 * instead of from DS server is that otherwise
 * it will cause deadlock
 */
  struct mproc *rmp;
  endpoint_t mucod_ep = 0;
  for (rmp = &mproc[0]; rmp < &mproc[NR_PROCS]; rmp++) {
        if (!(rmp->mp_flags & IN_USE))
                continue;
        if (!strcmp(rmp->mp_name, "mucod")) {
                mucod_ep = rmp->mp_endpoint;;
                return mucod_ep;
        }
  }
}
 ...
 *                              tell_mucod                                   *
 *===========================================================================*/
void tell_mucod(m_ptr)
message *m_ptr;
{
  int r;
  endpoint_t mucod_ep = get_mucod_id();
  fprintf(stderr, "\n");
  r = asynsend3(mucod_ep, m_ptr, AMF_NOREPLY);
  if (r != OK)
        panic("unable to send to MUCOD: %d", r);
}
